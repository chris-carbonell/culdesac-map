...
# get address for map center
address_center = st.text_input("Address", "5602 Schroeder Rd, Madison, WI")

# assume Madison, WI if necessary
if not(address_center.endswith(", WI")):
    address_center = address_center + ", Madison, WI"

# geocode
geolocator = Nominatim(user_agent="GTA Lookup")
geocode = RateLimiter(geolocator.geocode, min_delay_seconds=1)
location = geolocator.geocode(address_center)

# map
st.pydeck_chart(pdk.Deck(
    map_style="mapbox://styles/mapbox/light-v9",
    initial_view_state={"latitude": location.latitude,
                        "longitude": location.longitude, "zoom": 15, "pitch": 50},
...
st.markdown(f"- number of matching properties: {len(df_results.index)} (out of {len(df_data.index)})")